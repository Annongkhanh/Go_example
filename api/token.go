...
func (server *Server) renewAccessToken(ctx *gin.Context) {
	...
	requestPayload, err := server.tokenMaker.VerifyToken(req.RefreshToken)
	...
	session, err := server.store.GetSession(ctx, requestPayload.ID)
	...
	if session.Username != requestPayload.Username {
